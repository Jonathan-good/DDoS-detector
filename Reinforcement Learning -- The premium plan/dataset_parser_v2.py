...
def parse_packet(pkt):
    ...
    try:
        # 4. Append packet timestamp, length, highest layer, and IP flags to the features list:
        ...
        # the string representation of the highest_layer attribute of a network packet using the SHA-256 algorithm and
        # then convert the resulting hexadecimal hash into an integer.
        ...
        # int(..., 16): Converts the hexadecimal string into an integer using base 16 (hexadecimal).
        pf.features_list.append(int(int(pkt.ip.flags, 16)))  # IP flags

        # 5. Extract source and destination IP addresses and store them in tmp_id list:
        tmp_id[0] = str(pkt.ip.src)  # int(ipaddress.IPv4Address(pkt.ip.src))
        tmp_id[2] = str(pkt.ip.dst)  # int(ipaddress.IPv4Address(pkt.ip.dst))

        # 6. Transform protocols using a vectorizer and calculate their value:
        ...
        protocols = [1 if i >= 1 else 0 for i in
                     protocols]  # we do not want the protocols counted more than once
        # (sometimes they are listed twice in pkt.frame_info.protocols)
        protocols_value = int(np.dot(np.array(protocols), powers_of_two))
        pf.features_list.append(protocols_value)

        # 7. Extract transport layer protocol and port information
        protocol = int(pkt.ip.proto)
        tmp_id[4] = protocol
        # 8. Check the transport layer protocol and extract relevant features accordingly:
        if pkt.transport_layer != None:
            if protocol == socket.IPPROTO_TCP:
                ...
                pf.features_list.append(int(pkt.tcp.ack))  # TCP ack. ACK is a signal sent by a receiving device
...
def process_pcap(pcap_file, in_labels, max_flow_len, labelled_flows, max_flows=0, traffic_type='all',
                 time_window=TIME_WINDOW):
    start_time = time.time()  # Record start time for performance measurement
    temp_dict = OrderedDict()  # Initialize dictionary to store packet flows
    start_time_window = -1  # Initialize start time of time window

    pcap_name = pcap_file.split("/")[-1]  # Extract file name from path
    print("Processing file: ", pcap_name)  # Print the name of the file being processed

    cap = pyshark.FileCapture(pcap_file)  # Open the pcap file for processing
    for i, pkt in enumerate(cap):  # Iterate over each packet in the pcap file
        if i % 1000 == 0:  # Print progress every 1000 packets
            print(pcap_name + " packet #", i)

        # Update start_time_window to group packets captured within the time window
        ...
        # Parse the packet and store information in a packet flow dictionary
        pf = parse_packet(pkt)
        store_packet(pf, temp_dict, start_time_window, max_flow_len)

        # Check if maximum flow limit is reached
        if max_flows > 0 and len(temp_dict) >= max_flows:
            break  # Exit loop if maximum flow limit is reached

    # Apply labels to the packet flows based on the provided labelled flows and input labels
    apply_labels(temp_dict, labelled_flows, in_labels, traffic_type)

    # Print completion message along with processing time
...
# Function to store packet information in the packet flow dictionary
def store_packet(pf, temp_dict, start_time_window, max_flow_len):
    """
    Store packet information in the packet flow dictionary.

    Parameters:
        pf (PacketFeatures): Object containing features extracted from the packet.
    ...
    """
    if pf is not None:
        # Check if forward packet ID is in the dictionary and time window
        ...
        # Check if backward packet ID is in the dictionary and time window
        ...
        # If neither forward nor backward packet ID is in the dictionary
        else:
            if pf.id_fwd not in temp_dict and pf.id_bwd not in temp_dict:  # If both IDs are new
                temp_dict[pf.id_fwd] = {start_time_window: np.array([pf.features_list]), 'label': 0}
            elif pf.id_fwd in temp_dict and start_time_window not in temp_dict[
                pf.id_fwd]:  # If only forward ID is in dictionary
                temp_dict[pf.id_fwd][start_time_window] = np.array([pf.features_list])
            elif pf.id_bwd in temp_dict and start_time_window not in temp_dict[
                pf.id_bwd]:  # If only backward ID is in dictionary
                temp_dict[pf.id_bwd][start_time_window] = np.array([pf.features_list])
    return temp_dict  # Return updated packet flow dictionary


# Function to apply labels to flows based on labelled flows and traffic type
def apply_labels(flows, labelled_flows, labels, traffic_type):
    ...
    for five_tuple, flow in flows.items():
        if labels is not None:
            short_key = (five_tuple[0], five_tuple[2])  # For IDS2017/IDS2018 dataset, labels have shorter keys
            flow['label'] = labels.get(short_key, 0)  # Assign label to flow
        for flow_key, packet_list in flow.items():
            if flow_key != 'label':  # Calculate relative time with respect to the time of the first packet in the flow
                amin = np.amin(packet_list, axis=0)[0]
                packet_list[:, 0] = packet_list[:, 0] - amin
        # Check traffic type and label to include flows accordingly
        if traffic_type == 'ddos' and flow['label'] == 0:  # Include malicious flows
            continue
        elif traffic_type == 'benign' and flow['label'] > 0:  # Include benign flows
            continue
        else:
            labelled_flows.append((five_tuple, flow))  # Add flow to labelled flows list


# returns the total number of flows
def count_flows(preprocessed_flows):
    # Initialize counters
    ddos_flows = 0  # Counter for DDoS flows
    total_flows = len(preprocessed_flows)  # Total number of flows
    ddos_fragments = 0  # Counter for DDoS fragments
    total_fragments = 0  # Total number of fragments

    # Iterate through preprocessed flows
    for flow in preprocessed_flows:
        flow_fragments = len(flow[1]) - 1  # Number of fragments in the flow
        total_fragments += flow_fragments  # Update total fragments count
        if flow[1]['label'] > 0:  # If the flow is labeled as DDoS
            ddos_flows += 1  # Increment DDoS flows counter
            ddos_fragments += flow_fragments  # Update DDoS fragments count (the label does not count)

    # Return tuple containing counts
...
if __name__ == "__main__":
    main(sys.argv)
    # process_pcap('dataset/CIC-DDoS-2019-SynFlood.pcap', in_labels=parse_labels('DOS2018', label=1), max_flow_len=10, labelled_flows=[], max_flows=0, traffic_type='all', time_window=TIME_WINDOW)
